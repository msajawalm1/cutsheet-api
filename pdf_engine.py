...
Generates a Letter-size Fabric Cut Sheet PDF from passed-in data.
...
def build_group(group):
    items = []
    h = Table([[P(group.get('groupName',''), S(10, bold=True)),
...
def build_pdf_from_data(order, line_items, accessories, output):
    ...
    if accessories:
        ...
        acc_hdr = [P(h, S(8, bold=True)) for h in
